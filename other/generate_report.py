...
def create_plots(df):
    ...
    ax.set_title(
        "Распределение процентных ставок", fontsize=16, weight="bold", color="#2ABB00"
    )
    ...
    ax.set_title(
        "Распределение сроков (месяцы)", fontsize=16, weight="bold", color="#2ABB00"
    )
...
def generate_pdf_report(csv_path, output_path):
    ...
    font_path = (
        Path(__file__).parent / "DejaVuSerif.ttf"
    )  # Путь к TTF-шрифту (укажи, где у тебя лежит файл)
