...
def generate_presentation(csv_path, pptx_path):
    ...
    # Заглавный слайд
    add_title_slide(
        prs, "Отчёт по вкладам Сбербанка", "Автоматически сгенерированная презентация"
    )
    ...
    term_plot = create_hist_plot(
        df,
        "term_months",
        "Распределение сроков (месяцы)",
        "Срок (мес.)",
        "Количество вкладов",
    )
